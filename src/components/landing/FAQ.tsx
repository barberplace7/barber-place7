        ...
        {FAQ_DATA.map((faq, index) => (
          <div key={index} className="group bg-white rounded-lg sm:rounded-xl shadow-md hover:shadow-xl transition-all duration-300 border border-gray-100 hover:border-gray-300 overflow-hidden">
            <button
              ...
              className="w-full px-4 sm:px-6 md:px-8 py-3 sm:py-4 md:py-5 flex items-center justify-between hover:bg-gray-50 transition-all duration-300"
            >
              <h3 className={`text-sm sm:text-base font-bold text-gray-800 text-left group-hover:text-gray-900 transition-colors ${montserrat.className}`}>{faq.q}</h3>
              <div className={`w-7 h-7 sm:w-8 sm:h-8 rounded-full bg-gradient-to-br from-blue-50 to-purple-50 flex items-center justify-center flex-shrink-0 ml-3 sm:ml-4 group-hover:scale-110 transition-transform duration-300 ${openFaqIndex === index ? 'bg-gradient-to-br from-blue-100 to-purple-100' : ''}`}>
                <svg
                  className={`w-4 h-4 sm:w-5 sm:h-5 text-gray-600 transition-transform duration-300 ${openFaqIndex === index ? 'rotate-180' : ''}`}
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                >
            ...
            >
              <div className="px-4 sm:px-6 md:px-8 pb-3 sm:pb-4 md:pb-5 pt-2 bg-gray-50">
                <p className={`text-xs sm:text-sm text-gray-700 leading-relaxed ${montserrat.className}`}>{faq.a}</p>
              </div>
            </div>
          </div>
        ))}
