...
export default function CompleteVisitModal({ state, onClose }: any) {
  const handleSubmit = async () => {
    ...
    // Auto-set QRIS amount to service total
    const qrisAmount = state.paymentMethod === 'QRIS' ? total : undefined;
            ...
            <div className="p-4 bg-blue-50 border border-blue-200 rounded-lg">
              <div className="flex items-start gap-2">
                ...
                <div className="flex-1">
                  <p className="text-sm font-medium text-blue-800">Pembayaran QRIS</p>
                  <p className="text-xs text-blue-700 mt-1">
                    Nominal QRIS otomatis sesuai harga layanan: Rp {(state.completingCustomer.visitServices?.reduce((sum: number, vs: any) => sum + vs.service.basePrice, 0) || 0).toLocaleString()}
                  </p>
                </div>
              </div>
            </div>
