...
export default function KasirDashboard() {
  ...
  const [expandedMenus, setExpandedMenus] = useState<{[key: string]: boolean}>({ transactions: true });
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex h-screen overflow-hidden">
      {/* Sidebar */}
      <div className={`bg-white shadow-lg transition-all duration-300 ${sidebarOpen ? 'w-64' : 'w-0'} flex flex-col h-screen overflow-hidden`}>
        <div className="p-4 border-b border-gray-200">
          <div className="flex items-center justify-between">
            <div className={`flex items-center space-x-3 ${!sidebarOpen && 'justify-center'}`}>
              ...
              {sidebarOpen && (
                <div>
                  <h1 className="text-lg font-bold text-gray-900">Kasir {branchInfo.kasirName}</h1>
                  <p className="text-xs text-gray-500">{branchInfo.name}</p>
                </div>
              )}
            </div>
            {sidebarOpen && (
        ...
        <nav className="flex-1 p-4 space-y-2">
          ...
          {/* Live Transactions with Submenu */}
          <div>
            <button
              onClick={() => setActiveTab('transactions')}
              className={`w-full flex items-center justify-between p-3 rounded-lg text-left transition-colors ${
                activeTab === 'transactions' ? 'bg-black text-white' : 'hover:bg-gray-100'
              } ${sidebarOpen ? '' : 'justify-center'}`}
            >
              <div className="flex items-center space-x-3">
                <span className={activeTab === 'transactions' ? 'text-white' : 'text-gray-600'}>
                  <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 10V3L4 14h7v7l9-11h-7z" />
                  </svg>
                </span>
                {sidebarOpen && <span className="font-semibold">Live Transactions</span>}
              </div>
            </button>
            {sidebarOpen && (
          ...
          >
            ...
            {sidebarOpen && <span className="font-semibold">Transaction History</span>}
          </button>
        </nav>
      ...
      <div className="flex-1 overflow-auto">
        <div className={`p-6 transition-all duration-300 ${!sidebarOpen ? 'mt-16' : ''}`}>
